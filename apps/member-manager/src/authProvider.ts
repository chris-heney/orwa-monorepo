...
const fetchUserWithRole = async (
  ...
  const response = await fetch(
    `${apiEndpoint}/api/users/${userId}?populate=role`,
    {
      method: 'GET',
      headers: {
...
const authProvider: AuthProvider = {
    ...
    try {
      ...
      try {
        const userMetaResponse = await fetch(
          `${apiEndpoint}/api/users/me?populate=role`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (!getRoleName(userWithRole)) {
          userWithRole = await fetchUserWithRole(
            apiEndpoint,
            userData.user.id,
            userData.jwt
          );
  ...
  logout: async () => {
    ...
    try {
      ...
      const { IMPERSONATION_STORAGE_KEY } = await import(
  ...
  // not permitted" and must NOT log the user out.
  checkError: ({ status }) => {
    if (status === 401) {
      Cookies.deleteCookie('token');
      Cookies.deleteCookie('role');
