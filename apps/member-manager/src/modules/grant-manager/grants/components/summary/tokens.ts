...
/**
 * Design tokens for the grant financial summary.
 *
 * Duotone "water ledger" palette anchored in the RIG logo blue. Money moves
 * through a stage gradient: blue (requests) -> amber (commitments) ->
 * green (money moved). Red is reserved for deficits and exits.
 *
 * Two variants share one shape: DARK is the original night ledger; LIGHT is
 * a re-mix (not an inversion) — paper canvas, white panels, and every accent
 * darkened enough to hold AA-ish contrast on white.
 */
export interface SummaryTokens {
  ...
  hoverShadow: string; // card lift on hover
  watermarkOpacity: number; // RIG logo watermark treatment per canvas
...
export const DARK: SummaryTokens = {
  mode: "dark",
  ink: "#0D141B",
  panel: "#151F29",
  panelSoft: "#1C2936",
...
export const LIGHT: SummaryTokens = {
  mode: "light",
  ink: "#F2F7FB",
  panel: "#FFFFFF",
  panelSoft: "#E5EEF6",
  line: "rgba(16, 63, 92, 0.16)",
  ...
  hoverShadow: "0 8px 24px rgba(23, 60, 86, 0.16)",
...
/** Active token set for the current MUI palette mode. */
export const useSummaryTokens = (): SummaryTokens =>
