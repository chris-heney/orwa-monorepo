...
const SoonerwarnStatusFilter = () => {
  ...
  const handleStatusChange = (statusId: string) => {
    if (applicationStatuses.includes(statusId)) {
      ...
      setApplicationStatuses(applicationStatuses.filter((status) => status !== statusId))
    } else {
      // Select the status
  ...
  return (
    <Box p={2} width={isSmall ? '90%' : '100%'}>
      <RadioGroup value={applicationStatuses} onChange={(e) => handleStatusChange(e.target.value)}>
        ...
        {legendData?.map((legend, i) => (
          <FormControlLabel
            key={`status-${i}`}
            sx={{ backgroundColor: legend.color }}
            value={legend.id}
