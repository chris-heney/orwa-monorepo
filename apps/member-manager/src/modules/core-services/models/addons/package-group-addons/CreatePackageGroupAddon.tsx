...
const CreatePackageGroupAddon = ({
    ...
    const [packageGroupInputKey, setPackageGroupInputKey] = useState(0);

    console.log(record?.packageGroupAddons);
    ...
    // Move useGetList outside of any conditionals
    // Fetch existing package group features to check for duplicates
    const { data: packageGroupFeatures } = useGetList('package-group-addon', {
        pagination: { page: 1, perPage: 1000 },
        filter: { featureId: record?.id }, // Use optional chaining since record might be null
    });

    if (!record) return null;

    // I need an array of the packeges from the packageGroupAddons
    const packageGroupAddons = record.packageGroupAddons?.map((addon: any) => addon.packageGroupId);

    const validatePackageGroupFeature = async (values: any) => {
        const errors: Record<string, any> = {};

        // Required field validations
        if (!values.packageGroupId) {
            errors.packageGroupId = 'Package Group is required';
        }

        // Check for duplicate package group and feature combination
        if (values.packageGroupId && packageGroupFeatures) {
            const existingFeature = packageGroupFeatures.find(
                (feature: any) =>
                    feature.packageGroupId === values.packageGroupId &&
                    feature.featureId === record.id
            );

            if (existingFeature) {
                errors.packageGroupId =
                    'This feature is already assigned to this package group';
                // Force the packageGroupId field to rerender
                setTimeout(() => setPackageGroupInputKey(prev => prev + 1), 0);
            }
        }
        return errors;
    };
                ...
                <Grid2 container spacing={2} width="100%">
                    ...
                    <Grid2 size={{ xs: 12 }}>
                        <ReferenceInput
                            ...
                            key={packageGroupInputKey}
                            filter={{
                                'coreServices.id': {
                                    $eq: record.coreServiceId,
                                },
                                ...(packageGroupAddons?.length ? {
                                    'id': {
                                        $notIn: packageGroupAddons,
                                    },
                                } : {}),
                            }}
                        >
    ...
    return (
        <Create redirect={false} resource="package-group-addon">
            <ModalHeader
                title="Add Package Group Addon"
            ...
            <SimpleForm
                ...
                validate={validatePackageGroupFeature}
            >
                <CreateFormContent />
            </SimpleForm>
        </Create>
...
export default CreatePackageGroupAddon;
