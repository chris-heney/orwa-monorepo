...
const entityOptions = [
  ...
  {
    id: 'training-instructor-certification',
  ...
  {
    id: 'events-annual-conference-attendee-roster',
    name: 'Annual Conference Attendee Roster',
  },
  {
    id: 'events-annual-conference-booth-roster',
    name: 'Annual Conference Booth Roster',
  },
  { id: 'events-expo-attendee-roster', name: 'Expo Attendee Roster' },
  { id: 'events-expo-booth-roster', name: 'Expo Booth Roster' },
  {
    id: 'events-fall-conference-attendee-roster',
    name: 'Fall Conference Attendee Roster',
  },
  {
    id: 'events-fall-conference-booth-roster',
    name: 'Fall Conference Booth Roster',
  },
...
const ScheduledEmailTaskFormFields = () => {
  ...
  const filteredSavedQueries = useMemo(() => {
    return savedQueries.filter((query) => {
      // A query has no owner when it was created outside the UI, or when its
      // author was removed. Reading `.toString()` off that null took down the
      // whole page, so treat "no owner" as simply "not mine".
      const owner =
        typeof query.user === 'object' && query.user !== null
          ? query.user.id
          : query.user;
      const isMine =
        identity != null &&
        owner != null &&
        String(owner) === String(identity.id);
      const hasPermission = Boolean(query.is_public) || isMine;
  ...
  // Clear the saved query when the user switches entity — a query written for
  // water systems is meaningless against associates.
  //
  // Only on a real switch. `entity` arrives undefined and then settles to the
  // record's value on load, and treating that as a change wiped the link that
  // had just been restored, leaving the task looking unfiltered.
  const previousEntityRef = useRef<string | undefined>(undefined);
  useEffect(() => {
    const previousEntity = previousEntityRef.current;
    previousEntityRef.current = selectedEntity;

    if (previousEntity === undefined || previousEntity === selectedEntity) {
      return;
    }
